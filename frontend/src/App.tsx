...
import { Upload, Loader2, FileText, CheckCircle2, AlertCircle, Lightbulb, Play, Terminal } from 'lucide-react';
  ...
  const startAnalysis = async () => {
    ...
    try {
      ...
      while (true) {
        ...
        buffer = lines.pop() || ''; // Keep the last incomplete fragment in buffer
      ...
      console.error(err);
      setError(err.message || 'Connection failed. Please check your internet or try again later.');
      ...
      return (
        <div className="flex flex-col items-center justify-center h-96 space-y-6">
          ...
          <div className="text-center">
            ...
            <p className="text-slate-500 mt-2">{progressMsg || "Processing research data..."}</p>
            <p className="text-xs text-slate-400 mt-4 max-w-md mx-auto">
              Please do not close the tab. Analysis is running in real-time.
            </p>
          </div>
        </div>
      );
